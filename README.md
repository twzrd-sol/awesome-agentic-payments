...
- [TWZRD Agent Intel](https://intel.twzrd.xyz) - Trust-scoring MCP server for AI agents on Solana using x402. Free tools (`score_agent`, `preflight_check`) verify agent wallet identity on-chain; `get_trust_receipt` delivers a signed, verifiable receipt via x402 USDC micropayment on Solana. No accounts or API keys. Config: `{"mcpServers":{"twzrd-agent-intel":{"url":"https://intel.twzrd.xyz/mcp"}}}`
